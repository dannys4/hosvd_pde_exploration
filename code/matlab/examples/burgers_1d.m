function [w_save,x,t,e_conn] = burgers_1d(epsilon,q1,q2,Nx,Nt)
%-------------------------------------------------------------------------------
%  burgers_1d - Solves the 1D viscous Burgers equation using a standard Galerkin
%               finite element formulation.  Optionally allows for a replacement
%               of linear operator and the addition of control and noise terms.
%
%           w_t = - w w_x + epsilon w_{xx} + u(x,t) + D\eta(x,t), \Omega = (0,1)
%           w(x,0) = { q1 \sin(2 \pi x)+q2 \sin(4 \pi x), 0\leq x \leq .5; 
%                      0                                , otherwise        }
%           w(0,t) = w(1,t)  (periodic boundary conditions)
%
%           solution integrated from t=0 to t=T_{max}=1.
%
%
%  Copyright (c) 2013, Jeff Borggaard, Virginia Tech
%  Version: 1.0
%
%  Usage:    [w,x,t] = burgers_1d(epsilon,Nx,Nt)
%
%  Variables:
%              epsilon
...
%  Outputs:
%              w
%                        Nodal solution matrix ((N+1) rows x (Nt+1) columns)
%              x
%                        Nodal coordinates
%              t
